...
def shorten():
    ...
    if existing_url_record is None:
        short_url = shorten_url(4)
        insert_db('shorten', ('url', 'short'), (long_url, short_url))
    else:
        short_url = existing_url_record[0]

    return render_template('shortened.html', url=long_url, short=request.host+"/"+short_url)
...
#    app.run(host='0.0.0.0', port=4567) #, ssl_context=('ssl/cert.pem', 'ssl/key.pem')
    app.run()